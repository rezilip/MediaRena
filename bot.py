...
# ================= سرور وب (برای جلوگیری از خطای Render) =================
app = Flask(__name__)
@app.route('/')
def home():
...
def callback_query(call):
    ...
    if call.data == "verify_join":
        ...
        if not url:
            ...
            return
        bot.edit_message_text("⏳ **در حال استخراج فایل...** لطفاً صبور باشید.", chat_id, call.message.message_id, parse_mode="Markdown")
...
def core_downloader(message, url, action, dl_id):
    ...
    # اگر فایل کوکی در گیت‌هاب وجود داشته باشد، آن را متصل میکند تا ارور لاگین ندهد
    ...
    try:
        try:
            # تلاش اول برای دانلود با فرمت درخواستی
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=True)
        except yt_dlp.utils.DownloadError as e:
            # سیستم ضد ارور: اگر ارور فرمت داد، به صورت خودکار با قطعی‌ترین فرمت دوباره تلاش می‌کند
            if "Requested format is not available" in str(e):
                bot.edit_message_text("🔄 **در حال تنظیم خودکار فرمت سازگار...**", chat_id, msg_id, parse_mode="Markdown")
                ydl_opts['format'] = 'best'
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    info = ydl.extract_info(url, download=True)
            else:
                raise e # اگر ارور چیز دیگری بود آن را ثبت کن
        ...
        if file_size < 50 * 1024 * 1024:
            ...
            if cloud_url:
                ...
                bot.edit_message_text("❌ **خطا در آپلود ابری.**", chat_id, msg_id, parse_mode="Markdown")
        else:
            bot.edit_message_text("❌ **خطا:** حجم فایل بیشتر از ۲۰۰ مگابایت است. آپلودسنتر ما توانایی میزبانی فایل‌های بالای ۲۰۰ مگابایت را ندارد. لطفاً کیفیت پایین‌تری انتخاب کنید.", chat_id, msg_id, parse_mode="Markdown")

    except yt_dlp.utils.DownloadError as e:
        error_msg = str(e).replace('`', '')[:250]
        bot.edit_message_text(f"❌ **خطای دانلود:**\n`{error_msg}...`", chat_id, msg_id, parse_mode="Markdown")
