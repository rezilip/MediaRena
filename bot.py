...
BOT_TOKEN = "8659065494:AAEyt2G0QBUDi0icUCGhCMdBuNtzz-TsRCE" # حتماً توکن جدید بات فادر را بگذارید
ADMIN_ID = 8516792883
...
def callback_query(call):
    ...
    if call.data == "verify_join":
        ...
        if not url:
            bot.answer_callback_query(call.id, "❌ این درخواست منقضی شده است. لطفا لینک ویدیو را دوباره بفرستید.", show_alert=True)
            return
        bot.edit_message_text("⏳ **در حال استخراج و دریافت فایل...** لطفاً صبور باشید.", chat_id, call.message.message_id, parse_mode="Markdown")
...
def core_downloader(message, url, action, dl_id):
    ...
    ydl_opts = {
        ...
        'cookiefile': 'cookies.txt', # فایل کوکی برای دور زدن یوتیوب
        'extractor_args': {'youtube': {'player_client': ['android', 'web']}},
        'http_headers': {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    }

    # =========================================================
    # فرمت‌های کاملاً تضمینی و ضدگلوله (غیرممکن است ارور بدهد)
    # =========================================================
    if action in ["low", "360"]:
        ydl_opts['format'] = 'worst' # ضعیف‌ترین فایلی که همزمان صدا و تصویر دارد
    elif action in ["optimal", "best", "720"]:
        ydl_opts['format'] = 'b' # بهترین فایلی که از قبل صدا و تصویر به هم چسبیده دارد
    elif action in ["high", "1080"]:
        ydl_opts['format'] = 'bestvideo+bestaudio/best' # بالاترین کیفیت ممکن (در صورت نبود، حالت امن را می‌آورد)
    elif action == "mp3":
        ydl_opts['format'] = 'bestaudio/best'
        ydl_opts['postprocessors'] = [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192'}]
    else:
        ydl_opts['format'] = 'best' # حالت پشتیبان
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if file_size < 50 * 1024 * 1024:
                ...
                bot.edit_message_text("🚀 **حجم فایل بیش از ۵۰ مگابایت است.** در حال آپلود ابری (این مرحله ممکن است ۱ دقیقه زمان ببرد)...", chat_id, msg_id, parse_mode="Markdown")
                ...
                bot.edit_message_text("❌ **خطا:** حجم فایل بیشتر از ۲۰۰ مگابایت است و آپلودسنتر تلگرام/ابری گنجایش آن را ندارد. کیفیت پایین‌تری انتخاب کنید.", chat_id, msg_id, parse_mode="Markdown")
